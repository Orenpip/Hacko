...
async function loadDataOnPage() {
  ...
  try {
    ...
    gradesHeader.innerHTML = "<strong style='color: #4CAF50; font-size: 13px;'>📊 Current Grades</strong>";
    gradesHeader.style.marginBottom = "8px";
    content.appendChild(gradesHeader);
    ...
    courses.slice(0, 5).forEach(course => {
      ...
      div.innerHTML = `<strong>${course.name}</strong><br><span style="color: #4CAF50;">Grade: ${grade}</span>`;
      content.appendChild(div);
    });

    if (courses.length > 5) {
      const moreDiv = document.createElement("div");
      moreDiv.style.marginTop = "6px";
      moreDiv.style.fontSize = "11px";
      moreDiv.style.color = "#666";
      moreDiv.style.fontStyle = "italic";
      moreDiv.textContent = `+${courses.length - 5} more courses`;
